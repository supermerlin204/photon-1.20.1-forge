...
public final class PostEffectStack {
    ...
    private static Object lerpValue(@Nullable Object base, Object target, float t) {
        if (target instanceof Float f && base instanceof Float b) return b + (f - b) * t;
        if (target instanceof Vector2f v && base instanceof Vector2f b) return new Vector2f(b).lerp(v, t);
        if (target instanceof Vector3f v && base instanceof Vector3f b) return new Vector3f(b).lerp(v, t);
        if (target instanceof Vector4f v && base instanceof Vector4f b) return new Vector4f(b).lerp(v, t);
        if (target instanceof Integer argb && base instanceof Integer b) return lerpArgb(b, argb, t);
        return target;
    }
