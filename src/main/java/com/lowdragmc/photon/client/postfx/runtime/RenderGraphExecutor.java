...
public final class RenderGraphExecutor {
            ...
            for (int passIndex = 0; passIndex < effect.passes().size(); passIndex++) {
                ...
                if (pass.customShader() != null) {
                    ...
                    pass.params().forEach((name, binding) -> {
                        var value = resolveBinding(binding, params, weight);
    ...
    @Nullable
    private static Object resolveBinding(CompiledEffect.ValueBinding binding,
                                         Map<String, Object> params, float weight) {
        if (binding instanceof CompiledEffect.ValueBinding.Constant constant) return constant.value();
        if (binding instanceof CompiledEffect.ValueBinding.ParamRef ref) return params.get(ref.schemaParam());
        if (binding instanceof CompiledEffect.ValueBinding.EffectWeight) return weight;
        return null;
    }
