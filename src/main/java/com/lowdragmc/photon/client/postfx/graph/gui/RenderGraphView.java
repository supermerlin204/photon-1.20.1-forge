...
public class RenderGraphView extends KGGraphView {
    ...
    private void onCanvasDoubleClick(UIEvent event) {
        ...
        var editor = editorView.getFirstAncestorOfType(Editor.class);
        if (editor == null) return;
        var nestedView = new GraphEditorView(FullscreenGraphView::new)
                .loadGraph(inner, savedTag -> {
                    if (!FullscreenGraphLibrary.save(path, savedTag)) {
                        showReadOnlyNotification();
                    }
                });
        nestedView.setRootPath(path);
        nestedView.setCanRemove(true);
        nestedView.setDynamicName(pass::getDisplayName);
        nestedView.setIcon(FullscreenShaderGraphResource.INSTANCE.getIcon());
        editor.placeView(nestedView, () -> editor.centerWindow.getLeftTop());
