...
public class PhotonParticleManager extends ParticleManager implements ParticleTickHost {
    ...
    private void consumeEditorEffectsWithoutParticles(Predicate<ParticleRenderType> renderTypeFilter) {
        ...
        try {
            ...
            if (output != chain) {
                // The scene lives in a sub-viewport. Preserve both the clip box and whether clipping
                // was active; the editor intentionally suspends its UI scissor while particles run.
                boolean scissorEnabled = GL11.glIsEnabled(GL11.GL_SCISSOR_TEST);
                int[] scissorBox = new int[4];
                GL11.glGetIntegerv(GL11.GL_SCISSOR_BOX, scissorBox);
                try {
                    GlStateManager._enableScissorTest();
                    GlStateManager._scissorBox(viewportX, viewportY, viewportWidth, viewportHeight);
                    com.lowdragmc.photon.client.postfx.runtime.SceneBlit.writeBack(output, mainTarget);
                } finally {
                    GlStateManager._scissorBox(scissorBox[0], scissorBox[1],
