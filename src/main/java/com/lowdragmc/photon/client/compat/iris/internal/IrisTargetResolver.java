...
final class IrisTargetResolver {
    ...
    private @Nullable IrisFrameTarget resolveInternal(boolean translucentQueue,
        ...
        if (inRender) {
            // Do not gate on WorldRenderingPhase.PARTICLES. Oculus scopes it to vanilla's
            // ParticleEngine.render, while Photon can flush its queues from an adjacent Forge render
            // stage. isRenderingWorld plus "no editor scene manager" identifies the world pass.
            if (!accessor.photon$isRenderingWorld()) return fail("not rendering the world");
            if (PhotonParticleManager.getRenderingManager() != null) {
    ...
    private static boolean usesTranslucentProgram() {
        // Oculus 1.20.1 has a separate translucent particle phase, but not every shader pack
        // provides a meaningful PARTICLES_TRANS program, so keep this an explicit compatibility opt-in.
    ...
    private IrisFrameTarget probe(IrisRenderingPipeline pipeline,
        ...
        try {
            ...
            return new IrisFrameTarget(fbo.getId(), attachmentTextures, colortexIndices,
                    ...
                    renderTargets == null ? depthTexture : renderTargets.getDepthTexture(),
     ...
     */
    private static boolean accumulatesIntoPrimary(@Nullable ShaderInstance shader) {
        if (shader instanceof ExtendedShaderAccessor extended) {
    ...
    private static String shaderKindOf(@Nullable ShaderInstance shader) {
