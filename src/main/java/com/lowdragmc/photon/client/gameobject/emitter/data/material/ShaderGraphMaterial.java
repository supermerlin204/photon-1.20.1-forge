...
public class ShaderGraphMaterial extends ShaderInstanceMaterial {
    ...
    private static void applyOverride(@Nullable KGMaterialValues values,
        ...
        } else if (value instanceof RenderTypeGraphTypes.GradientValue gradient) {
            values.setGradient(name, gradient);
        } else if (value instanceof RenderTypeGraphTypes.CurveValue curve) {
            values.setCurve(name, curve);
        } else if (value instanceof HDRColor hdr) {
            values.setHDRColorUniform(name,
                    new Vector4f(hdr.getR(), hdr.getG(), hdr.getB(), hdr.getIntensity()));
        } else if (value instanceof Vector2f v) {
            values.setUniform(name, v);
        } else if (value instanceof Vector3f v) {
            values.setUniform(name, v);
        } else if (value instanceof Vector4f v) {
            values.setUniform(name, v);
        } else if (value instanceof Float f) {
            values.setByVariable(name, f);
        } else if (value instanceof Boolean b) {
            values.setByVariable(name, b ? 1f : 0f);
    ...
    private static Tag encodeValue(Object value) {
        ...
        if (value instanceof Vector2f v) return floatList(v.x, v.y);
        if (value instanceof Vector3f v) return floatList(v.x, v.y, v.z);
        if (value instanceof Vector4f v) return floatList(v.x, v.y, v.z, v.w);
        if (value instanceof RenderTypeGraphTypes.Sampler2DValue sampler) {
            return typed("sampler", RenderTypeGraphTypes.SAMPLER2D_CODEC.encodeStart(NbtOps.INSTANCE, sampler)
                    .result().orElse(null));
        }
        if (value instanceof RenderTypeGraphTypes.GradientValue gradient) {
            return typed("gradient", RenderTypeGraphTypes.GRADIENT_CODEC.encodeStart(NbtOps.INSTANCE, gradient)
                    .result().orElse(null));
        }
        if (value instanceof RenderTypeGraphTypes.CurveValue curve) {
            return typed("curve", RenderTypeGraphTypes.CURVE_CODEC.encodeStart(NbtOps.INSTANCE, curve)
                    .result().orElse(null));
        }
        if (value instanceof HDRColor hdr) {
            return typed("hdr_color", HDRColor.CODEC.encodeStart(NbtOps.INSTANCE, hdr).result().orElse(null));
        }
        return null;
    }
    ...
    private static Object copyValue(@Nullable Object value) {
        if (value instanceof RenderTypeGraphTypes.GradientValue gradient) return gradient.copy();
        if (value instanceof RenderTypeGraphTypes.CurveValue curve) return curve.copy();
