    ...
    protected void drawSeriesCurve(GuiGraphics graphics, ECBCurves value, int armPoint, float x, float y, float width, float height) {
        ...
        if (curves.get(0).p0.x > 0) {
            DrawerHelper.drawLines(graphics, List.of(
                            toScreen(new Vector2f(0, curves.get(0).p0.y), x, y, width, height),
                            toScreen(curves.get(0).p0, x, y, width, height)),
                    ColorPattern.T_RED.color, ColorPattern.T_RED.color, 0.3f);
        }
        if (curves.get(curves.size() - 1).p1.x < 1) {
            DrawerHelper.drawLines(graphics, List.of(
                            toScreen(new Vector2f(1, curves.get(curves.size() - 1).p1.y), x, y, width, height),
                            toScreen(curves.get(curves.size() - 1).p1, x, y, width, height)),
                    ColorPattern.T_RED.color, ColorPattern.T_RED.color, 0.3f);
        }
