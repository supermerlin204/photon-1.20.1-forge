...
/**
 * The HDR counterpart of {@link RandomColor}: a random blend between two HDR vectors. The blend
...
public class HDRRandomColor implements HDRColorFunction {
    ...
    public HDRRandomColor() {
        this(HDRColorCompat.black(), HDRColorCompat.white());
    }
    ...
    public void setColorA(Vector4f color) {
        this.colorA = color == null ? HDRColorCompat.black() : color;
    }

    public void setColorB(Vector4f color) {
        this.colorB = color == null ? HDRColorCompat.white() : color;
    }

    @Override
    public void loadConfig(NumberFunctionConfig config) {
        var color = (int) config.defaultValue();
        this.colorA = HDRColorCompat.fromARGB(color);
        this.colorB = HDRColorCompat.fromARGB(color);
    }

    @Override
    public void sampleHDR(float t, Supplier<Float> lerp, Vector4f out) {
        var a = HDRColorCompat.premultiplied(colorA);
        var b = HDRColorCompat.premultiplied(colorB);
        var blend = lerp.get();
    ...
    public NumberFunction copy() {
        return new HDRRandomColor(new Vector4f(colorA), new Vector4f(colorB));
    }
    ...
    public void createConfigurator(NumberFunctionConfigurator configurator) {
        ...
        configurator.inlineContainer.addChild(new UIElement().layout(layout -> {
            ...
            layout.wrap(FlexWrap.WRAP);
                ...
                }, new Vector4f(colorA), true),
                b = new HDRColorConfigurator("", () -> colorB, color -> {
                    setColorB(color);
                    configurator.updateValue(this);
                }, new Vector4f(colorB), true)
        ));
        a.layout(layout -> {
            layout.flex(1);
            layout.minWidth(40);
