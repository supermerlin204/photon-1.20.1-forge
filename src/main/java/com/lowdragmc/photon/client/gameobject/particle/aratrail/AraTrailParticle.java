    ...
    private void emissionStep(float time) {
        ...
        if (accumTime >= runtime.timeInterval.get()) {
            if (config.emit) {
                ...
                if (points.isEmpty() || (
                        position.distance(points.get(points.size() - 1).position) >= runtime.minDistance.get()
                )) {
                    emitPoint(position);
                    accumTime = 0;
                }
...
//        }
        ...
        if (!points.isEmpty())
            texcoord = points.get(points.size() - 1).texcoord
                    + position.distance(points.get(points.size() - 1).position);
