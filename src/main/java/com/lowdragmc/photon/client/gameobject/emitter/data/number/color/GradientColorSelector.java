...
public class GradientColorSelector extends BindableUIElement<GradientColor> {
    ...
    public GradientColorSelector(boolean hdr) {
        this.hdr = hdr;
        this.intensityConfigurator = hdr ? new NumberConfigurator("HDR", () -> selectedIntensity,
        ...
        if (selectedPoint >= 0) {
            if (isSelectAlpha && selectedPoint < value.getAP().size()) {
                ...
                if (hdr) {
                    ...
                    var split = HDRColorCompat.fromPremultiplied(rgb.y, rgb.z, rgb.w);
                    selectedIntensity = split.w;
                    colorSelector.setColor(HDRColorCompat.baseARGB(split), false);
                } else {
