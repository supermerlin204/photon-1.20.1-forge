...
public class RenderPassPipeline {
    @Nullable
    @Getter
    private static RenderPassPipeline collecting = null;
    ...
    private final int[] entryScissorBox = new int[4];
    ...
    public static void finishCollecting(RenderPassPipeline pipeline) {
        try {
            pipeline.build();
        } finally {
            if (collecting == pipeline) collecting = null;
        }
    }
    ...
    public void build() {
        ...
        try {
            ...
            if (!completed) {
                ...
                // commonly use particlesTarget or a visual-layer target).  Restore the exact entry
                // target/viewport so the caller's next batch cannot be redirected or clipped.
                try {
                    restoreEntryState();
    ...
    private void prepareTarget(int width, int height) {
        ...
        DRAW_TARGET.bindWrite(false);
        // RenderTarget.bindWrite resets the viewport to the full accumulator. Keep the scene's
        // original sub-viewport for particle projection and rasterization.
    ...
    private void afterRendering() {
        ...
        var mainTarget = UISurface.currentTarget();
        // Use the viewport captured before DRAW_TARGET was bound. The active viewport at this
        // point is usually the accumulator's full-size viewport, not the embedded scene rectangle.
        var lastViewport = entryViewport;
        ...
        boolean uiScissorEnabled = false;
        int[] uiScissorBox = null;
        if (hasDifferentViewPort) {
            uiScissorEnabled = GL11.glIsEnabled(GL11.GL_SCISSOR_TEST);
            uiScissorBox = new int[4];
            GL11.glGetIntegerv(GL11.GL_SCISSOR_BOX, uiScissorBox);
            GlStateManager._enableScissorTest();
            GlStateManager._scissorBox(lastViewport.position.x, lastViewport.position.y,
        ...
        try {
            ...
            // Restore both halves of the caller's clip state. Restoring only the box while
            // unconditionally disabling the test made a focused editor element change how every
            // later UI texture in the frame was rasterized.
            if (uiScissorBox != null) {
                GlStateManager._scissorBox(uiScissorBox[0], uiScissorBox[1],
                        uiScissorBox[2], uiScissorBox[3]);
                if (uiScissorEnabled) {
                    GlStateManager._enableScissorTest();
                } else {
    ...
    private void restoreEntryState() {
        ...
        GlStateManager._scissorBox(entryScissorBox[0], entryScissorBox[1],
                entryScissorBox[2], entryScissorBox[3]);
        if (entryScissorEnabled) {
            GlStateManager._enableScissorTest();
        } else {
            GlStateManager._disableScissorTest();
        }
