...
public abstract class Emitter extends FXObject implements IParticleEmitter {
    ...
    protected void update(float dt) {
        ...
        if (getLifetime() > 0) {
            if(isLooping())
                ...
                t = com.lowdragmc.photon.util.MathCompat.clamp(this.ageF / getLifetime(), 0f, 1f);
        }
    }
    ...
    public float getT(float partialTicks) {
        if (this.lifetime > 0){
            if (!isLooping()) return com.lowdragmc.photon.util.MathCompat.clamp(
                    t + partialTicks / this.lifetime, 0f, 1f);
    ...
    public AABB getRenderBoundingBox(float partialTicks) {
        ...
        return cullBox == null ? INFINITE_BOUNDS : cullBox;
    }
