...
public class CustomData {
    ...
    private CustomData(Type type, int channelCount, List<NumberFunction> channels, List<String> names) {
        this.type = type;
        this.channelCount = com.lowdragmc.photon.util.MathCompat.clamp(channelCount, 1, MAX_CHANNELS);
    ...
    public void setChannelCount(int count) {
        this.channelCount = com.lowdragmc.photon.util.MathCompat.clamp(count, 1, MAX_CHANNELS);
    ...
    private void resizeNames() {
        while (channelNames.size() > channels.size()) {
            channelNames.remove(channelNames.size() - 1);
        }
        while (channelNames.size() < channels.size()) {
            channelNames.add("");
        }
    ...
    private static NumberFunction toHDR(NumberFunction function) {
        if (function instanceof HDRColorFunction) return function;
        if (function instanceof Color color) {
            return new HDRConstantColor(HDRColorCompat.fromARGB(color.getNumber().intValue()));
        }
        if (function instanceof RandomColor random) {
            return new HDRRandomColor(HDRColorCompat.fromARGB(random.getA().intValue()),
                    HDRColorCompat.fromARGB(random.getB().intValue()));
        }
        if (function instanceof Gradient gradient) {
            return new HDRGradient(gradient.getGradientColor().copy());
        }
        if (function instanceof RandomGradient random) {
            return new HDRRandomGradient(random.getGradientColor0().copy(),
    ...
    public static CustomData fromNBT(CompoundTag tag) {
        ...
        int channelCount = com.lowdragmc.photon.util.MathCompat.clamp(
                tag.getInt("channelCount"), 1, MAX_CHANNELS);
