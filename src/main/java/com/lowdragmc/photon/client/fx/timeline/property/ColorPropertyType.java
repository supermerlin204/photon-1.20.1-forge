...
public class ColorPropertyType implements AnimatedPropertyType {
    ...
    public int captureColor(FXObject target) {
        return HDRColorCompat.toARGB(captureHDRColor(target));
    }
     ...
     */
    public Vector4f captureHDRColor(FXObject target) {
        ...
        try {
            if (value instanceof HDRColorFunction hdr) {
                ...
                return HDRColorCompat.fromPremultiplied(sample);
            }
            if (value instanceof NumberFunction function) {
                return HDRColorCompat.fromARGB(function.get(0f, () -> 0f).intValue());
            }
        } catch (Exception ignored) {
        }
        return HDRColorCompat.white();
    }
    ...
    public AnimatedProperty create(FXObject target) {
        ...
        property.addStop(0, HDRColorCompat.baseARGB(captured), captured.w);
        return property;
    }
