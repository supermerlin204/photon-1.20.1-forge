...
public class AnimatedProperty {
    ...
    private static boolean isSingleKey(ECBCurves curve) {
        ...
        return segs.size() == 1 && segs.get(0).p0.equals(segs.get(0).p1);
    }
    ...
    public Vector2f key(int axis, int k) {
        var segs = channels[axis].getSegments();
        if (k <= 0) return new Vector2f(segs.get(0).p0);
        if (k >= segs.size()) return new Vector2f(segs.get(segs.size() - 1).p1);
    ...
    public int addKey(int axis, float tick, float value) {
        ...
        if (isSingleKey(channels[axis])) {
            var existing = new Vector2f(segs.get(0).p0);
        ...
        if (tick <= segs.get(0).p0.x) {
            if (tick == segs.get(0).p0.x) return -1;
            segs.add(0, makeSegment(p, new Vector2f(segs.get(0).p0)));
            return 0;
        }
        if (tick >= segs.get(segs.size() - 1).p1.x) {
            if (tick == segs.get(segs.size() - 1).p1.x) return -1;
            segs.add(makeSegment(new Vector2f(segs.get(segs.size() - 1).p1), p));
            return segs.size();
        }
