...
public class ColorAnimatedProperty extends AnimatedProperty {
    ...
    public int sampleColor(float time) {
        ...
        if (stops.size() == 1) return stops.get(0).argb;
    ...
    public float sampleIntensity(float time) {
        ...
        if (stops.size() == 1) return stops.get(0).intensity;
        var first = stops.get(0);
        var last = stops.get(stops.size() - 1);
    ...
    public NumberFunction sampleFunction(float time) {
        ...
        if (hdr) {
            return new HDRConstantColor(HDRColorCompat.withIntensity(
                    HDRColorCompat.fromARGB(sampleColor(time)), sampleIntensity(time)));
        }
