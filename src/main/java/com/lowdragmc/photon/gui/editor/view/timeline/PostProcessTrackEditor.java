...
public class PostProcessTrackEditor extends ClipTrackEditor {
    ...
    private static PostProcessClip.ParamKind kindOf(CompiledEffect.ParamSpec spec) {
        ...
        if (value instanceof Float) return PostProcessClip.ParamKind.FLOAT;
        if (value instanceof Boolean) return PostProcessClip.ParamKind.BOOL;
        // An Integer default is a COLOR (ARGB, lerpable) or an INT parameter.
        if (value instanceof Integer) {
            return spec.lerpable() ? PostProcessClip.ParamKind.COLOR : PostProcessClip.ParamKind.INT;
        }
        if (value instanceof Vector2f) return PostProcessClip.ParamKind.VEC2;
        if (value instanceof Vector3f) return PostProcessClip.ParamKind.VEC3;
        if (value instanceof Vector4f) return PostProcessClip.ParamKind.VEC4;
        if (value instanceof RenderTypeGraphTypes.Sampler2DValue) return PostProcessClip.ParamKind.SAMPLER;
        return null;
    }
