    ...
    public class ParticleSceneEditor extends SceneEditor {
        ...
        protected void renderAfterWorld(@NotNull MultiBufferSource bufferSource, float partialTicks) {
            if (fxObjectInfoView.getInspected() != null) {
                // Shape guides, force-field ranges and the cull box use immediate custom line
                // buffers. They must not leave their shader/blend/VAO state active for the
                // MultiBufferSource batch or for the resource UI drawn after this scene.
                try (var state = UIRenderStateScope.capture()) {
                    ...
                    if (isCullBoxVisible && fxObjectInfoView.getInspected() instanceof FXObject fxObject) {
                        ...
                        if (cullBox != FXObject.INFINITE_BOUNDS) {
