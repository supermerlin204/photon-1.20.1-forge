...
import com.lowdragmc.photon.gui.editor.widget.ReorderableTreeList;
...
public class FXHierarchyView extends View {
    ...
    public final ReorderableTreeList<FXObjectTreeNode> treeList = new ReorderableTreeList<>();
    ...
    private void applyReparent(List<IFXObject> moving, Transform target, Transform newParent,
                               TreeList.DropMode mode) {
        for (var k : moving) {
        ...
        if (mode == TreeList.DropMode.INTO) {
            ...
            int insertPos = (ti < 0) ? rest.size()
                    : (mode == TreeList.DropMode.BEFORE ? ti : ti + 1);
    ...
    private List<IFXObject> copySceneObject(IFXObject toCopied) {
        ...
        for (var child : toCopied.children()) {
            if (child instanceof IFXObject childFXObject) {
                ...
                copiedChildren.get(0).transform().parent(copied.transform(), false);
