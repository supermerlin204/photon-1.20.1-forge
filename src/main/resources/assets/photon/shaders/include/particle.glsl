...
ParticleData getParticleData() {
    ...
    // Forge 1.20.1: N points along -Z, so cross(N, T) = uDir * -Y and handedness is -uDir * vDir.
    data.Tangent = vec4(normalize(rotMat * vec3(uDir, 0, 0)), -uDir * vDir);
    // 1.20.1 billboards face -Z before rotation (matching vanilla's particle quad winding).
